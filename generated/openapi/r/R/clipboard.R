...
Clipboard <- R6::R6Class(
  ...
  public = list(
    ...
    #' @export
    isValid = function() {
      if (!str_detect(self$`source`, "[a-zA-Z0-9_][a-zA-Z0-9_-]{0,31}\\\\.(?!-)[a-zA-Z0-9-]{1,63}(?<!-)$")) {
        return(FALSE)
      }
    ...
    getInvalidFields = function() {
      invalid_fields <- list()
      if (!str_detect(self$`source`, "[a-zA-Z0-9_][a-zA-Z0-9_-]{0,31}\\\\.(?!-)[a-zA-Z0-9-]{1,63}(?<!-)$")) {
        invalid_fields["source"] <- "Invalid value for `source`, must conform to the pattern [a-zA-Z0-9_][a-zA-Z0-9_-]{0,31}\\\\.(?!-)[a-zA-Z0-9-]{1,63}(?<!-)$."
      }
