#' Create a new GraphRelationship
#'
#' @description
...
#' @description GraphRelationship Class
...
#' @field data  \link{GraphRelationshipAllOfData} [optional]
...
GraphRelationship <- R6::R6Class(
  ...
  public = list(
    ...
    #' Initialize a new GraphRelationship class.
    #'
    #' @description
    #' Initialize a new GraphRelationship class.
    #'
    ...
    initialize = function(`id`, `source`, `specversion`, `type`, `datacontenttype` = NULL, `dataschema` = NULL, `subject` = NULL, `time` = NULL, `data` = NULL, `data_base64` = NULL, additional_properties = NULL, ...) {
      ...
      if (!missing(`source`)) {
        if (!(`source` %in% c("graph.relationship"))) {
          stop(paste("Error! \"", `source`, "\" cannot be assigned to `source`. Must be \"graph.relationship\".", sep = ""))
        }
    ...
    #' To JSON String
    #'
    #' @return GraphRelationship in JSON format
    #' @export
    toJSON = function() {
      GraphRelationshipObject <- list()
      if (!is.null(self$`id`)) {
        GraphRelationshipObject[["id"]] <-
          self$`id`
      }
      if (!is.null(self$`source`)) {
        GraphRelationshipObject[["source"]] <-
          self$`source`
      }
      if (!is.null(self$`specversion`)) {
        GraphRelationshipObject[["specversion"]] <-
          self$`specversion`
      }
      if (!is.null(self$`type`)) {
        GraphRelationshipObject[["type"]] <-
          self$`type`
      }
      if (!is.null(self$`datacontenttype`)) {
        GraphRelationshipObject[["datacontenttype"]] <-
          self$`datacontenttype`
      }
      if (!is.null(self$`dataschema`)) {
        GraphRelationshipObject[["dataschema"]] <-
          self$`dataschema`
      }
      if (!is.null(self$`subject`)) {
        GraphRelationshipObject[["subject"]] <-
          self$`subject`
      }
      if (!is.null(self$`time`)) {
        GraphRelationshipObject[["time"]] <-
          self$`time`
      }
      if (!is.null(self$`data`)) {
        GraphRelationshipObject[["data"]] <-
          self$`data`$toJSON()
      }
      if (!is.null(self$`data_base64`)) {
        GraphRelationshipObject[["data_base64"]] <-
          self$`data_base64`
      }
      for (key in names(self$additional_properties)) {
        GraphRelationshipObject[[key]] <- self$additional_properties[[key]]
      }

      GraphRelationshipObject
    },
    #' Deserialize JSON string into an instance of GraphRelationship
    #'
    #' @description
    #' Deserialize JSON string into an instance of GraphRelationship
    #'
    #' @param input_json the JSON input
    #' @return the instance of GraphRelationship
    #' @export
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`source`)) {
        if (!is.null(this_object$`source`) && !(this_object$`source` %in% c("graph.relationship"))) {
          stop(paste("Error! \"", this_object$`source`, "\" cannot be assigned to `source`. Must be \"graph.relationship\".", sep = ""))
        }
      ...
      if (!is.null(this_object$`data`)) {
        `data_object` <- GraphRelationshipAllOfData$new()
    ...
    fromJSONString = function(input_json) {
      ...
      self$`subject` <- this_object$`subject`
      self$`time` <- this_object$`time`
      self$`data` <- GraphRelationshipAllOfData$new()$fromJSON(jsonlite::toJSON(this_object$`data`, auto_unbox = TRUE, digits = NA))
      self$`data_base64` <- this_object$`data_base64`
      # process additional properties/fields in the payload
      for (key in names(this_object)) {
        if (!(key %in% self$`_field_list`)) { # json key not in list of fields
    ...
    #' Validate JSON input with respect to GraphRelationship
    #'
    #' @description
    #' Validate JSON input with respect to GraphRelationship and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`id`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GraphRelationship: the required field `id` is missing."))
      }
      ...
      if (!is.null(input_json$`source`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GraphRelationship: the required field `source` is missing."))
      }
      ...
      if (!is.null(input_json$`specversion`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GraphRelationship: the required field `specversion` is missing."))
      }
      ...
      if (!is.null(input_json$`type`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for GraphRelationship: the required field `type` is missing."))
      }
    },
    ...
    #' To string (JSON format)
    #'
    #' @return String representation of GraphRelationship
    #' @export
    toString = function() {
      self$toJSONString()
    },
...
# GraphRelationship$unlock()
#
## Below is an example to define the print function
# GraphRelationship$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# GraphRelationship$lock()
