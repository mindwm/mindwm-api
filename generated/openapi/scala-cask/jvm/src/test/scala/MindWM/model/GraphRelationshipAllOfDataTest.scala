...
class GraphRelationshipAllOfDataTest extends AnyWordSpec with Matchers {

    "GraphRelationshipAllOfData.fromJson" should {
        """not parse invalid json""" in {
           val Failure(err) = Try(GraphRelationshipAllOfDataData.fromJsonString("invalid jason"))
           err.getMessage should startWith ("Error parsing json 'invalid jason'")
        }
        """parse """ ignore {
           val Failure(err : ValidationErrors) = GraphRelationshipAllOfDataData.fromJsonString("""""").validated()
