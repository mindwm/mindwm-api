-module(MindWM_server).


-define(DEFAULT_LOGIC_HANDLER, MindWM_default_logic_handler).
...
get_socket_transport(IP, Port, Options) ->
    ...
    case MindWM_utils:get_opt(ssl, Options) of
        SslOpts = [_|_] ->
            {ssl, Opts ++ SslOpts};
        undefined ->
            {tcp, Opts}
...
get_default_dispatch(LogicHandler) ->
    Paths = MindWM_router:get_paths(LogicHandler),
